...
void set_save_file(){
    ...
    // Um einen Spielstand zu laden muss eine CSV Datei im Ordner existieren, die Spielstand.csv heißt
    while (1)
    {
        printf("Spielstand laden? y/n ");
//        set_cursor(9,2);
//        printf("Um einen Spielstand zu laden muss eine CSV Datei\n im Ordner existieren, die Spielstand.csv heißt");
...
void play_game()
{
    ...
    while (1)
    {
        ...
        if (!check_if_cell_was_deleted(row, col)) {
            printf("Feld kann nicht ver\x84ndert werden");
            continue;
        }
